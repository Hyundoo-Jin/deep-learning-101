...
def get_annotation(breed, filename) :
    target_file = os.path.join(ANNOTATION_DIR, breed, filename)
    with open(target_file, 'r') as f :
        doc = BeautifulSoup(f.read(), 'lxml')
    return doc

def get_bounding_size(breed, filename):
    doc = get_annotation(breed, filename.split(".")[0])
    size_info = get_orginal_size_info(breed, filename)
    ...
    for box in bndboxes:
        ...
        bounding_width = width if size_info["width"] > (xmin+width) else size_info["width"] - xmin
        bounding_height = height if size_info["height"] > (ymin+height) else size_info["height"] - ymin
...
def get_orginal_size_info(breed, filename):
    doc = get_annotation(breed, filename.split(".")[0])
    size_info = {}
...
flags = tf.app.flags
flags.DEFINE_string("image_dir", "Images", "The directory of dog images [Images]")    # flags는 argument들을 저장하는 클래스
...
def convert_image_data_to_tfrecords(
    x_data, y_data, data_type, split_index=128):

    TAGET_DIR = get_target_dir()
    ...
    for images_filename, y_label in zip(x_data, y_data):
        ...
        try:
            ...
            image_list = []
            for box in get_bounding_size(y_label, images_filename) :
                xmin, ymin, bounding_width, bounding_height = box
                image = tf.image.crop_to_bounding_box(image, offset_height = ymin, offset_width = xmin, target_height = bounding_height, target_width = bounding_width)
        ...
        for image in image_list:
            try:
                ...
                feature = {'label': bytes_feature(image_label),
...
def main(_):
    ...
    convert_image_data_to_tfrecords(X_train, y_train, data_type=TRAIN, split_index=128)
    convert_image_data_to_tfrecords(X_test, y_test, data_type=TEST, split_index=128)
